...
- Si agregas un módulo nuevo al panel (ej. cuando se construya SEO), agrégalo a `ModuleKey` aquí Y a `moduleOptions`/`ModuleKey` en `frontend-kw` (`src/features/admin/users/schemas/user.schema.ts` y `types.ts`) — deben quedar alineados, igual que los defaults de homepage.
